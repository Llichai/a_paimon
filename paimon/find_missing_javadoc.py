...
paimon_core_path = r"D:\a_git\paimon\paimon-core"
java_files = []
...
for file_path in sorted(java_files):
    if not has_class_javadoc(file_path):
        ...
        missing_javadoc.append(relative_path)
...
# 按包名分组显示
packages = {}
for file_path in missing_javadoc:
...
for package in sorted(packages.keys()):
    print(f"\n{package}:")
    for file in sorted(packages[package]):
        print(f"  - {file}")
