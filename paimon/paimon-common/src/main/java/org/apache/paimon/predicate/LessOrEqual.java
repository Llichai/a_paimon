...
/**
 * 小于等于(Less Or Equal)比较函数,用于判断字段值是否小于或等于常量值。
 *
 * <p>该类实现了 SQL 中的小于等于比较操作(<=),例如: {@code age <= 60}, {@code score <= 100.0}
 *
 * <h2>比较逻辑</h2>
 * <ul>
 *   <li><b>精确测试</b>: 使用类型感知的比较器判断字段值是否小于或等于常量值</li>
 *   <li><b>统计信息测试</b>: 如果 min <= literal,返回 true(可能存在 <= literal 的值);
 ...
 *
 * <h2>谓词否定</h2>
 * LessOrEqual 的否定是 {@link GreaterThan}:
 * <ul>
 ...
 * // 如果 min=60, 则 age<=60 可能存在(可能所有值都是60),返回 true
 * }</pre>
 *
 * <h2>设计考虑</h2>
 * <ul>
 ...
 *   <li>与 {@link LessThan} 的区别在于包含等于的情况</li>
 * </ul>
 *
 * @see GreaterOrEqual 大于等于比较函数
 * @see LessThan 小于比较函数
 * @see GreaterThan 大于比较函数(LessOrEqual 的否定形式)
...
public class LessOrEqual extends NullFalseLeafBinaryFunction {
     ...
     * 基于统计信息进行快速过滤测试。
     *
     * <p>该方法判断是否可能存在小于或等于常量值的数据:
     * <ul>
     *   <li>如果 min <= literal,返回 true(可能存在 <= literal 的值)</li>
     *   <li>如果 min > literal,返回 false(所有值都 > literal,不存在 <= literal 的值)</li>
     * </ul>
     *
     * <p>示例:
     * <ul>
     ...
     *   <li>age <= 60, min=60: 60 <= 60,返回 true (与 LessThan 的区别)</li>
     * </ul>
     *
     * @param type 字段的数据类型
     ...
     * @param literal 常量值
     * @return true 表示可能存在小于或等于常量值的数据;false 表示确定不存在小于或等于常量值的数据
     */
    @Override
    public boolean test(
