...
/**
 * 大于(Greater Than)比较函数,用于判断字段值是否大于常量值。
 *
 * <p>该类实现了 SQL 中的大于比较操作(>),例如: {@code age > 18}, {@code price > 100.0}
 *
 * <h2>比较逻辑</h2>
 * <ul>
 *   <li><b>精确测试</b>: 使用类型感知的比较器判断字段值是否大于常量值</li>
 *   <li><b>统计信息测试</b>: 如果 max > literal,返回 true(可能存在大于 literal 的值);
 ...
 *
 * <h2>谓词否定</h2>
 * GreaterThan 的否定是 {@link LessOrEqual}:
 * <ul>
 ...
 * @see LessOrEqual 小于等于比较函数(GreaterThan 的否定形式)
...
public class GreaterThan extends NullFalseLeafBinaryFunction {
     ...
     * 基于统计信息进行快速过滤测试。
     *
     * <p>该方法判断是否可能存在大于常量值的数据:
     * <ul>
     *   <li>如果 max > literal,返回 true(可能存在大于 literal 的值)</li>
     *   <li>如果 max <= literal,返回 false(所有值都 <= literal,不存在大于 literal 的值)</li>
     * </ul>
     *
     * <p>示例:
     * <ul>
     ...
     *   <li>age > 18, max=18: 18 <= 18,返回 false</li>
     * </ul>
     *
     * @param type 字段的数据类型
     ...
     * @param literal 常量值
     * @return true 表示可能存在大于常量值的数据;false 表示确定不存在大于常量值的数据
     */
    @Override
    public boolean test(
