 ...
 * 数据演化分片读取器
 *
 * <p>这是一个联合 {@link SplitRead} 实现，用于跨Schema版本读取数据文件并合并列。
 *
 * <h2>数据演化支持</h2>
 * <p>Paimon 支持Schema演化（添加列、删除列、修改类型等），该类负责处理跨不同Schema版本的数据读取：
 * <ul>
 *   <li><b>向后兼容</b>：新Schema可以读取旧Schema写入的数据
 *   <li><b>列合并</b>：将分散在不同文件中的列数据合并成完整记录
 *   <li><b>类型转换</b>：处理列类型变更的数据转换
 *   <li><b>字段映射</b>：建立物理文件字段与逻辑表字段的映射关系
 * </ul>
 *
 * <h2>Schema演化场景</h2>
 * <p>支持以下Schema变更场景：
 * <ol>
 *   <li><b>添加列</b>：新列在旧文件中使用默认值或NULL
 *   <li><b>删除列</b>：忽略旧文件中已删除的列
 *   <li><b>重命名列</b>：通过字段ID追踪列的重命名
 *   <li><b>类型提升</b>：如 INT -> BIGINT
 * </ol>
 *
 * <h2>Blob文件处理</h2>
 * <p>Blob文件是特殊的列式文件，每个文件只包含一个或几个列：
 * <ul>
 *   <li>支持列级别的增量更新
 *   <li>减少小范围更新的写入放大
 *   <li>需要在读取时合并多个Blob文件
 ...
 *
 * <h2>限制</h2>
 * <p>注意：该类不支持以下功能（因为它们会干扰列合并过程）：
 * <ul>
 *   <li>过滤器下推（Predicate Push Down）
 *   <li>删除向量（Deletion Vectors）
 * </ul>
 *
 * <h2>TODO</h2>
 * <p>优化该类的实现。
 *
 * @see SplitRead 分片读取接口
 * @see DataEvolutionFileReader 数据演化文件读取器
 */
public class DataEvolutionSplitRead implements SplitRead<InternalRow> {
     ...
     * @param schemaManager Schema管理器
     * @param schema 当前表Schema
     * @param rowType 要读取的行类型
     ...
     * 设置过滤器（不支持）
     *
     * <p>TODO: 支持文件索引下推（所有条件）和谓词下推（仅在没有列合并时）
     *
