...
/**
 * FieldRoaringBitmap64Agg 聚合器工厂类
 * 用于创建 RBM64 聚合函数，使用 Roaring Bitmap 进行64位长整数的位图压缩和去重
 */
public class FieldRoaringBitmap64AggFactory implements FieldAggregatorFactory {

    public static final String NAME = "rbm64"; // 聚合函数标识符
     ...
     * @param field 字段名称
     * @return FieldRoaringBitmap64Agg 聚合器实例
     */
    @Override
    public FieldRoaringBitmap64Agg create(DataType fieldType, CoreOptions options, String field) {
        ...
        checkArgument(
                ...
                fieldType);
        // 创建并返回 RBM64 聚合器，用于64位长整数的高效位图存储
