 ...
 * 值和位置持久化处理器
 *
 * <p>返回 {@link PositionedKeyValue} 的 {@link PersistProcessor}。
 *
 * <p>特点：
 * <ul>
 *   <li>持久化：存储值 + 行位置 + 序列号 + RowKind
 *   <li>读取：返回 PositionedKeyValue（完整键值对 + 位置信息）
 *   <li>需要行位置信息
 * </ul>
 *
 * <p>使用场景：
 * <ul>
 *   <li>LOOKUP changelog 模式 + Deletion Vector：同时需要值和位置
 *   <li>行级别更新：定位并更新特定行
 *   <li>完整信息查询：需要返回值和原始位置
 * </ul>
 *
 * <p>存储格式：
 * <pre>
 * [序列化的值 | 行位置(8字节) | 序列号(8字节) | RowKind(1字节)]
 * </pre>
 *
 * <p>示例：
 * <pre>
 * // 写入：key -> [value_bytes | rowPos=1234 | seq=100 | kind=+I]
 * // 查询：key -> PositionedKeyValue(KeyValue(key, 100, +I, value), "file.parquet", 1234)
 * </pre>
 */
public class PersistValueAndPosProcessor implements PersistProcessor<PositionedKeyValue> {
     ...
     * 持久化到磁盘
     *
     * <p>格式：[值字节 | 行位置(8字节) | 序列号(8字节) | RowKind(1字节)]
     *
     * @param kv 键值对
    ...
    public byte[] persistToDisk(KeyValue kv, long rowPosition) {
        ...
        byte[] bytes = new byte[vBytes.length + 8 + 8 + 1]; // 值 + 行位置 + 序列号 + RowKind
        MemorySegment segment = MemorySegment.wrap(bytes);
        segment.put(0, vBytes); // 写入值
        segment.putLong(bytes.length - 17, rowPosition); // 写入行位置
    ...
    public PositionedKeyValue readFromDisk(
        ...
        MemorySegment segment = MemorySegment.wrap(bytes);
        long rowPosition = segment.getLong(bytes.length - 17); // 读取行位置
        long sequenceNumber = segment.getLong(bytes.length - 9); // 读取序列号
