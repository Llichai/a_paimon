 ...
 * 数据演化压缩任务序列化器
 *
 * <p>DataEvolutionCompactTaskSerializer 负责序列化和反序列化
 * {@link DataEvolutionCompactTask},用于任务持久化和网络传输。
 *
 * <p>序列化格式:
 * <pre>
 * 字段顺序:
 * 1. partition (BinaryRow)
 * 2. compactBefore (List<DataFileMeta>)
 * 3. blobTask (boolean)
 * </pre>
 *
 * <p>版本控制:
 * <ul>
 *   <li>当前版本:CURRENT_VERSION = 1
 *   <li>版本不兼容:不同版本不能相互反序列化
 *   <li>强制重启:版本变更时需要重新启动作业,不能从 savepoint 恢复
 * </ul>
 *
 * <p>列表序列化:
 * 支持序列化和反序列化任务列表:
 * <ul>
 *   <li>{@code serializeList}: 序列化多个任务
 *   <li>{@code deserializeList}: 反序列化多个任务
 *   <li>格式:长度(int) + 任务1 + 任务2 + ...
 * </ul>
 *
 * <p>使用场景:
 * <ul>
 ...
 * @see VersionedSerializer 版本化序列化器接口
 */
