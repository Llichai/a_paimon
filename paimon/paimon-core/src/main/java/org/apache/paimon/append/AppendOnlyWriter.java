 ...
 * Append-Only 写入器
 *
 * <p>AppendOnlyWriter 是 Append-Only 表的核心写入器,只支持追加写入,不进行合并操作。
 * 这是 Paimon 中用于高吞吐量、只追加场景的写入实现。
 *
 * <p>与 KeyValue 表的区别:
 * <ul>
 *   <li><b>KeyValue 表</b>:需要主键,写入时合并(MergeTree),支持更新删除,适合 OLTP 场景
 *   <li><b>Append-Only 表</b>:无主键要求,只追加,不支持更新删除,性能更高,适合日志/事件流场景
 * </ul>
 *
 * <p>写入流程:
 * <pre>
 ...
 * 写入后台自动触发压缩任务,合并小文件:
 * <ul>
 *   <li>触发条件:小文件数量 > {@code compaction.min-file-num}
 *   <li>压缩策略:合并多个小文件为一个大文件
 *   <li>压缩协调器:{@link AppendCompactCoordinator} 或 {@link BucketedAppendCompactManager}
 *   <li>强制压缩:用户调用 {@code compact(true)} 时立即执行全量压缩
 * </ul>
 *
 * <p>Blob 字段支持:
 * 如果表包含 BLOB 类型字段,写入器会:
 * <ul>
 *   <li>使用 {@link RollingBlobFileWriter} 分离写入普通字段和 Blob 字段
 *   <li>为 Blob 字段生成独立的文件(*.blob)
 *   <li>在文件元数据中记录 Blob 文件的关联关系
 * </ul>
 *
 * <p>内存管理:
 * 实现 {@link MemoryOwner} 接口,支持动态内存管理:
 * <ul>
 *   <li>通过 {@link MemorySegmentPool} 分配内存页
 *   <li>内存不足时自动溢写到磁盘(Spillable Buffer)
 *   <li>支持设置最大磁盘溢写大小({@code write-buffer-spillable.max-disk-size})
 * </ul>
 *
 * <p>使用示例:
 * <pre>{@code
 * // 创建写入器
 * AppendOnlyWriter writer = new AppendOnlyWriter(
 *     fileIO,
 *     ioManager,
 *     schemaId,
 *     fileFormat,
 ...
 * }</pre>
 *
 * @see AppendCompactCoordinator Append 表压缩协调器
 * @see BucketedAppendCompactManager 分桶 Append 压缩管理器
 * @see RollingBlobFileWriter Blob 文件滚动写入器
 * @see BatchRecordWriter 批量记录写入器接口
 * @see MemoryOwner 内存所有者接口
 */
