 ...
 * 增量聚类管理器
 *
 * <p>IncrementalClusterManager 负责为 Unaware-Bucket 模式的 Append-Only 表
 * 创建和管理增量聚类任务,通过空间排序优化查询性能。
 *
 * <p>增量聚类:
 * 不是一次性对所有数据排序,而是逐层聚类:
 * <pre>
 * 快照 1: 写入 10 个文件 → Level 0
 * 快照 2: 聚类 Level 0 → Level 1 (5个文件,局部有序)
 * 快照 3: 写入 8 个文件 → Level 0
 * 快照 4: 聚类 Level 0+1 → Level 2 (3个文件,全局有序)
 * </pre>
 *
 * <p>核心流程:
 * <pre>
 * 1. 构建 LSM 结构(constructLevels):
 *    - 读取快照中的所有数据文件
 *    - 按分区分组文件
 *    - 为每个分区构建层级结构
 *
 * 2. 生成聚类单元(createCompactUnits):
 *    - 为每个分区调用 IncrementalClusterStrategy.pick
 *    - 选择需要聚类的文件和输出层级
 *    - 处理历史分区(HistoryPartitionCluster)
 *
 * 3. 转换为数据分片(toSplitsAndRewriteDvFiles):
 *    - 将聚类单元转换为 DataSplit
 *    - 处理删除向量,生成索引文件变更
 *    - 返回分片和提交消息
 *
 ...
 *
 * <p>空间排序曲线:
 * 支持三种排序策略({@link CoreOptions.OrderType}):
 * <ul>
 *   <li><b>HILBERT</b>:Hilbert 曲线,适合多维数据,局部性最好
 *   <li><b>ZORDER</b>:Z-Order 曲线,计算效率高,局部性较好
 *   <li><b>ORDER</b>:普通排序,简单但可能跳跃性大
 * </ul>
 *
 * <p>历史分区聚类:
 * {@link HistoryPartitionCluster} 处理长时间未更新的分区:
 * <ul>
 *   <li>空闲时间 > clustering.history-partition-idle-time
 *   <li>层级 < maxLevel (未达到最高层级)
 *   <li>限制数量:clustering.history-partition-limit
 * </ul>
 *
 * @see IncrementalClusterStrategy 增量聚类策略
 * @see HistoryPartitionCluster 历史分区聚类
 * @see Sorter 排序器(Hilbert/ZOrder/Order)
 */
