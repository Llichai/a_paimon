 ...
 * 分桶追加表的压缩管理器
 *
 * <p>BucketedAppendCompactManager 负责管理 Append-Only 表单个桶(bucket)的压缩任务,
 * 维护待压缩文件队列,并根据策略触发压缩。
 *
 * <p>与 {@link AppendCompactCoordinator} 的区别:
 * <ul>
 *   <li><b>AppendCompactCoordinator</b>:跨快照扫描,处理多个分区,适合独立压缩作业
 *   <li><b>BucketedAppendCompactManager</b>:单桶管理,嵌入写入流程,适合在线压缩
 * </ul>
 *
 * <p>压缩模式:
 * <pre>
 * 1. 自动压缩(triggerCompactionWithBestEffort):
 *    - 从 toCompact 队列中选择文件
 *    - 选择策略:滑动窗口,累计大小 >= targetFileSize
 *    - 文件数量 >= minFileNum 时触发
 *
 * 2. 全量压缩(triggerFullCompaction):
 *    - 压缩所有待压缩文件
 *    - 移除大文件(fileSize >= targetFileSize 且无删除向量)
 *    - 条件:small > big 且 size >= FULL_COMPACT_MIN_FILE(3)
 *
 * 3. 强制全量压缩:
 *    - 用户调用 compact(true) 时触发
 *    - 即使文件很少也会压缩(如果 forceRewriteAllFiles=true)
 * </pre>
 *
 * <p>文件选择策略(pickCompactBefore):
 * <pre>
 * 滑动窗口算法:
 * 1. 按 minSequenceNumber 排序文件(保证时间顺序)
 * 2. 从队列头部取文件,累计大小和数量
 * 3. 如果 fileNum >= minFileNum,触发压缩
 * 4. 如果 totalSize >= targetFileSize,移除窗口最左文件
 * 5. 重复直到队列为空
 * </pre>
 *
 * <p>删除向量支持:
 * 当表启用删除向量时:
 * <ul>
 *   <li>有删除向量的文件必须参与压缩,即使文件很大
 *   <li>压缩后调用 {@link BucketedDvMaintainer#removeDeletionVectorOf} 清理旧的删除向量
 *   <li>生成新的删除文件({@link CompactDeletionFile})
 * </ul>
 *
 * <p>压缩结果处理:
 * <ul>
 *   <li>如果压缩后最后一个文件仍然很小(< targetFileSize),加回队列头部
 *   <li>下次压缩时会优先选择这个小文件
 *   <li>避免产生大量小文件
 * </ul>
 *
 * <p>压缩任务类型:
 * <ul>
 *   <li>{@link FullCompactTask}:全量压缩任务,处理所有文件
 *   <li>{@link AutoCompactTask}:自动压缩任务,处理选中的文件
 * </ul>
 *
 * <p>使用示例:
 * <pre>{@code
 * // 创建压缩管理器
 * BucketedAppendCompactManager manager = new BucketedAppendCompactManager(
 *     executor,
 *     restoredFiles,
 *     dvMaintainer,
 *     minFileNum,
 ...
 *     files -> compactRewrite(files), // rewriter
 *     metricsReporter
 * );
 *
 * // 添加新文件
 ...
 * @see CompactFutureManager 压缩Future管理器基类
 * @see BucketedDvMaintainer 分桶删除向量维护器
 */
