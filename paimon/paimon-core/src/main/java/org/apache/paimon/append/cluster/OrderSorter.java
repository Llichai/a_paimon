 ...
 * 顺序排序器
 *
 * <p>OrderSorter 使用普通字典序对数据进行排序,
 * 按照聚类键的自然顺序排列数据。
 *
 * <p>排序方式:
 * <pre>
 * 聚类键: [region, city]
 * 排序结果: 先按 region 排序,再按 city 排序
 *
 * Asia, Beijing
 * Asia, Shanghai
 * Europe, London
 ...
 * 使用代码生成的投影提升性能:
 * <ul>
 *   <li>通过 {@link org.apache.paimon.codegen.CodeGenUtils} 生成投影代码
 *   <li>避免反射和动态查找
 *   <li>性能优于通用投影
 * </ul>
 *
...
public class OrderSorter extends Sorter {
    ...
    public OrderSorter(
            ...
            IOManager ioManager) {
        super(reader, keyType(valueType, orderColNames), valueType, options, ioManager);
    ...
    private static RowType keyType(RowType valueType, List<String> orderColNames) {
        List<String> fieldNames = valueType.getFieldNames();
        int[] keyProjectionMap = orderColNames.stream().mapToInt(fieldNames::indexOf).toArray();
        return addKeyNamePrefix(Projection.of(keyProjectionMap).project(valueType));
    }
    ...
    private static class OrderKeyAbstract implements KeyAbstract<InternalRow> {
        ...
        public OrderKeyAbstract(RowType rowType, List<String> orderColNames) {
            ...
            this.keyProjectionMap = orderColNames.stream().mapToInt(fieldNames::indexOf).toArray();
        }
