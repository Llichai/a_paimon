 ...
 * 分桶追加表的层级结构
 *
 * <p>BucketedAppendLevels 存储 Append-Only 表的所有层级文件,
 * 支持 LSM 树结构的层级管理。
 *
 * <p>层级结构:
 * <pre>
 * Level 0: 多个单文件 SortedRun (新写入,未排序)
 *   - file1.parquet (SortedRun)
 *   - file2.parquet (SortedRun)
 *   - file3.parquet (SortedRun)
 *
 * Level 1: 一个 SortedRun (多个文件,部分有序)
 *   - SortedRun([file4, file5, file6])
 *
 * Level 2: 一个 SortedRun (多个文件,全局有序)
 *   - SortedRun([file7, file8])
 * </pre>
 *
 * <p>关键特性:
 * <ul>
 *   <li>Level 0:每个文件是一个独立的 {@link SortedRun}
 *   <li>Level 1+:每个层级只有一个 {@link SortedRun},包含多个文件
 *   <li>层级数量:numLevels = levels.size() + 1 (Level 0 + levels)
 * </ul>
 *
 * <p>核心操作:
 * <ul>
 *   <li>{@code addLevel0File}: 添加新文件到 Level 0
 *   <li>{@code runOfLevel}: 获取指定层级的 SortedRun
 *   <li>{@code update}: 更新层级,移除 before 文件,添加 after 文件
 *   <li>{@code levelSortedRuns}: 返回所有层级的 LevelSortedRun 列表
 * </ul>
 *
 * @see LevelSortedRun 层级排序运行
 * @see SortedRun 排序运行
 * @see BucketedAppendClusterManager 聚类管理器
 */
