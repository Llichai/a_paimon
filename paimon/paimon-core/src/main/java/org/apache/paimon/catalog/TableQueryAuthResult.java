 ...
 * 表查询授权结果
 *
 * <p>TableQueryAuthResult 封装了表查询的授权信息,包括行级过滤和列掩码规则。
 * 用于实现细粒度的数据访问控制。
 *
 * <p>支持的授权类型:
 * <ul>
 *   <li><b>行级过滤（Row Level Filter）</b>: 限制用户只能查询满足条件的行
 *   <li><b>列掩码（Column Masking）</b>: 对敏感列进行脱敏处理
 * </ul>
 *
 * <p>行级过滤:
 * <p>通过 SQL 谓词表达式限制查询结果:
 * <pre>{@code
 * // 示例: 用户只能查询自己部门的数据
 * filter = ["department = 'engineering'"]
 *
 * // 示例: 多个过滤条件（AND 组合）
 * filter = ["department = 'engineering'", "level >= 3"]
 * }</pre>
 *
 * <p>列掩码:
 * <p>对敏感列进行脱敏处理:
 ...
 * // 示例: 掩码电话号码和邮箱
 * columnMasking = {
 *   "phone": "MASK(phone, 3, 4)",      // 只显示中间4位
 *   "email": "SUBSTRING(email, 1, 3)"  // 只显示前3个字符
 * }
 * }</pre>
 *
 * <p>工作流程:
 * <ol>
 *   <li>Catalog 调用 {@link Catalog#authTableQuery} 获取授权结果
 *   <li>授权结果包装成 {@link QueryAuthSplit}
 *   <li>读取器应用行级过滤和列掩码
 * </ol>
 *
 * <p>使用示例:
 ...
 * List<String> select = Arrays.asList("id", "name", "salary");
 * TableQueryAuthResult authResult = catalog.authTableQuery(identifier, select);
 *
 * // 2. 应用到查询计划
 * TableScan tableScan = table.newScan();
 * TableScan.Plan plan = tableScan.plan();
 * TableScan.Plan authPlan = authResult.convertPlan(plan);
 *
 * // 3. 读取数据（自动应用过滤和掩码）
 * for (Split split : authPlan.splits()) {
 ...
 * <p>集成场景:
 * <ul>
 *   <li><b>Apache Ranger</b>: 从 Ranger 获取行级过滤和列掩码策略
 *   <li><b>自定义授权</b>: 实现自定义的授权逻辑
 *   <li><b>多租户</b>: 实现租户间数据隔离
 * </ul>
 *
 * <p>性能考虑:
 * <ul>
 *   <li>行级过滤会增加查询开销（需要评估每一行）
 *   <li>列掩码需要额外的计算（如字符串截取）
 *   <li>建议在 Catalog 层做授权检查缓存
 * </ul>
 *
 * @see Catalog#authTableQuery
 * @see QueryAuthSplit
 */
